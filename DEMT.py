...
#    S0 = C0**-1
    S0 = np.linalg.inv(C0)
    q = ((alpha)/((1.-alpha**2)**1.5))*(np.arccos(alpha) - alpha*((1.-alpha**2)**0.5))
    S1111 = ((3.*alpha**2)/(8.*(1.-v)*alpha**2-1.)) + (1./(4*(1-v)))*(1-2.*v-(9./(4*(alpha**2-1.))))*q
    S3333 = (1./(2*(1-v)))*(1-2*v+(3.*alpha**2-1.)/(alpha**2-1.)-(1-2.*v+(3.*alpha**2)/(alpha**2-1.))*q)
    S1122 = (1./(4.*(1.-v)))*(((alpha**2)/(2*(alpha**2-1.)))-(1.-2*v+((3.)/(4.*(alpha**2-1.))))*q)
    S1133 = (1./(2.*(1.-v)))*((-alpha**2/(alpha**2-1.))+0.5*(((3.*alpha**2)/(alpha**2-1.))-(1.-2.*v))*q)
    S3311 = (1./(2.*(1.-v)))*(2.*v-1.-(1./(alpha**2-1.)) + (1.-2.*v+3./(2*(alpha**2-1.)))*q)
    S1212 = (1./(4.*(1.-v)))*((alpha**2/(2.*(alpha**2-1)))+(1.-2.*v-(3./(4.*(alpha**2-1))))*q)
    S1313 = (1./(4.*(1.-v)))*(1.-2.*v-((alpha**2+1.)/(alpha**2-1.))-0.5*(1.-2.*v-((3*(alpha**2+1))/(alpha**2-1.)))*q)
...
def Ctotal (C0, tr, vr, gr, vs=0., ts=0.):
#    I4_ = Id4()    
#    I4 = Kelvin(I4_)
#    tr = 
    I4 = np.identity(6)
...
#    print water,elemento
    [Ct, rho, phi, poisct] = IncludeIso (quartzo, clay, alphai, porosf, porosi, ni)
